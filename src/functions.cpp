...
int set(std::vector<Particle>& particles,
  ...
  if (std::fabs(particles[i].x) < RDISK) {
    r = 0;
  }
  for (int j = 1; j <= i - 1; j++) {
...
void status_initialize(std::vector<Particle>& particles) {
  ...
  for (size_t i = 0; i < particles.size(); i++) {
    ...
    particles[i].event.time = 2.0 * TEND;
...
int getcell_y(double y, double cell_length_y) {
  if (y < YMIN) {
    ...
    return NCELLY - 1;  // YMAXよりも高い位置の粒子は一番高いセルに登録
  } else {
...
void cell_register(std::vector<Particle>& particles,
  ...
  for (int cell_x = 0; cell_x < NCELLX; cell_x++) {
    for (int cell_y = 0; cell_y < NCELLY; cell_y++) {
      cells[cell_x][cell_y].first = -1;
      cells[cell_x][cell_y].last = -1;
    }
  }

  double cell_length_x = (XMAX - XMIN) / (double)NCELLX,
         cell_length_y = (YMAX - YMIN) / (double)NCELLY;
  for (size_t i = 0; i < particles.size(); i++) {
    const auto cell_x = getcell_x(particles[i].x, cell_length_x);
    const auto cell_y = getcell_y(particles[i].y, cell_length_y);
    const auto lastPrev = cells[cell_x][cell_y].last;
...
double T_DWC(const Particle& particle, double t, int j) {
  ...
  if (j == -1) {  // collision with RIGHT wall(-1)
    if (particle.u > 0.0) {
      tau += (XMAX - RDISK - particle.x) / particle.u;
    } else {
      tau += 2.0 * TEND;
    }
  } else if (j == -2) {  // collision with LEFT wall(-2)
    if (particle.u < 0.0) {
      tau += (XMIN + RDISK - particle.x) / particle.u;
    } else {
      tau += 2.0 * TEND;
    }
  } else if (j == -3) {  // collision with BOTTOM wall(-3)
    tau += (particle.v + sqrt(particle.v * particle.v +
                              2 * GRAVITY * (particle.y - RDISK))) /
           GRAVITY;
  }
  if (tau < t) {
    return 2.0 * TEND;
...
struct Event Predictions(std::vector<Particle>& particles,
  ...
  for (int c1 = -1; c1 < 1; c1++) {
    for (int c2 = -1; c2 <= 1; c2++) {
      if ((((cell_x + c1 >= 0) && (cell_x + c1 <= NCELLX - 1)) &&
           (cell_y + c2 >= 0)) &&
          (cell_y + c2 <= NCELLY - 1)) {
...
void CBT_build(std::vector<std::vector<Node>>& nodes,
  ...
  for (int i = 0; i < 2 * P + 2 * Q; i++) {
    if (i < 2 * P) {
      nodes[BTD][i].time = particles[i].event.time;
      nodes[BTD][i].number = i;
    } else {
      nodes[BTD][i].time = particles[2 * P + (i - 2 * P) / 2].event.time;
      nodes[BTD][i].number = 2 * P + (i - 2 * P) / 2;
    }
  }

  // tournament
  for (int n_index = BTD - 1; n_index >= 0; n_index--) {
...
void CBT_update(std::vector<std::vector<Node>>& entry, double time_new,
  ...
  Node* entry_now;
  if (i_new < 2 * P) {
    entry[BTD][i_new].time = time_new;
    entry_now = &entry[BTD][i_new];
  } else {
    entry[BTD][2 * i_new - 2 * P].time = time_new;
    entry[BTD][2 * i_new - 2 * P + 1].time = time_new;
    entry_now = &entry[BTD][2 * i_new - 2 * P];
  }
...
void G1(Particle& particle, int j) {
  if ((j == -1) || (j == -2)) {  // collision with R or L wall
    particle.u = -WREST * particle.u;
    if (j == -1) {
      particle.x = XMAX - RDISK - EPS;
      //このepsilon処理はgetcell_xのときなどに必要になる
    } else {
      particle.x = XMIN + RDISK + EPS;
    }
  } else if (j == -3) {  // collision with Bottom wall
    particle.v = (1 + WREST) * U - WREST * particle.v;
    particle.y = YMIN + RDISK + EPS;
  }
}
...
void G2(Particle& p1, Particle& p2) {
  ...
  p1.u = 0.5 * (1 + PREST) * ((u2 - u1) * cx + (v2 - v1) * cy) * cx + u1;
  p1.v = 0.5 * (1 + PREST) * ((u2 - u1) * cx + (v2 - v1) * cy) * cy + v1;
  p2.u = 0.5 * (1 + PREST) * ((u1 - u2) * cx + (v1 - v2) * cy) * cx + u2;
  p2.v = 0.5 * (1 + PREST) * ((u1 - u2) * cx + (v1 - v2) * cy) * cy + v2;
}
...
double t_cell_update(const Particle& particle, int j_current, double t_cell_old,
  ...
  dt_cell = (cell_length_y - 2.0 * RDISK) / (2.0 * (v_max));
...
void MaskUpdate(std::vector<Particle>& particles,
  ...
  for (int c1 = -1; c1 <= 1; c1++) {
    for (int c2 = -1; c2 <= 1; c2++) {
      if ((((cell_x + c1 >= 0) && (cell_x + c1 < NCELLX)) &&
           (cell_y + c2 >= 0)) &&
          (cell_y + c2 < NCELLY)) {
        j = cells[cell_x + c1][cell_y + c2].first;
        while (j >= 0) {
